...
FROM(
    SELECT department, count(*) AS total_count, SUM(CASE WHEN termdate <> "000-00-00" AND termdate <= curdate() THEN 1 ELSE 0 END) AS terminated_count
    FROM hr 
    WHERE age >= 18
    GROUP BY department
    ) AS subquery
...
-- 8. What is the distribution of employees across cities and states?
...
FROM(
  SELECT
    YEAR(hire_date) AS year, count(*) AS hires, SUM(CASE WHEN termdate <> "0000-00-00" AND termdate <= curdate() THEN 1 ELSE 0 END) AS terminations
    FROM hr
    WHERE age >= 18
